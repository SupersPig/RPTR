...
class CocoDetection(torchvision.datasets.CocoDetection):
    ...
    def __getitem__(self, idx):
        ...
        img, target = self.prepare(img, target, self.coco.cats)
...
import cv2
root_path = "H:\DataSets\RelTR/oi/images/"
class ConvertCocoPolysToMask(object):
    def __init__(self, return_masks=False):
        self.return_masks = return_masks

    def __call__(self, image, target, cats):
        w, h = image.size
        ...
        # img = cv2.imread(root_path+str(image_id.item())+'.jpg')
        # for idx, j in enumerate(boxes):
        #     _ = j[0].item()
        #     cv2.rectangle(img, (int(j[0].item()), int(j[1].item())), (int(j[2].item()), int(j[3].item())), (0, 0, 255), 2)
        #     temp = cats[classes[idx].item()]
        #     cv2.putText(img, str(temp['name']),  (int(j[0].item()), int(j[1].item())), cv2.FONT_HERSHEY_COMPLEX, 2.0, (100, 200, 200), 3)
        # cv2.imshow("image_{}".format(image_id), img)
        # cv2.waitKey()
