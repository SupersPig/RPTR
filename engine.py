...
from lib.evaluation.sg_eval_LGTR import BasicSceneGraphEvaluator, calculate_mR_from_evaluator_list
...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, args):
    ...
    for samples, targets in metric_logger.log_every(data_loader, 100, header):
        ...
        outputs = model(samples)
        # outputs = model.predcls(samples, targets)
        # loss_dict = criterion(outputs, targets)
        # weight_dict = criterion.weight_dict
...
def evaluate_rel_batch(outputs, targets, evaluator, evaluator_list):
    for batch, target in enumerate(targets):
        ...
        # 到这里把预测值pred_entry和真值gt_entry都提出来了
        evaluator['sgdet'].evaluate_scene_graph_entry(gt_entry, pred_entry)
        # evaluator['so_pair'].evaluate_scene_graph_entry(gt_entry, pred_entry)
        # evaluator['sgcls'].evaluate_scene_graph_entry(gt_entry, pred_entry)
        ...
        if evaluator_list is not None:
            for pred_id, _, evaluator_rel in evaluator_list:
                ...
                # evaluator_rel['sgcls'].evaluate_scene_graph_entry(gt_entry_rel, pred_entry)
