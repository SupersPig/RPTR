...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, entity_embed, triplet_embed, pos_embed, so_embed):
        ...
        hs, hs_t = self.decoder(entity, triplet, memory, memory_key_padding_mask=mask,
                                pos=pos_embed, entity_pos=entity_embed,
                                triplet_pos=triplet_embed, so_pos=so_embed)

        return hs.transpose(1, 2), hs_t.transpose(1, 2), memory.permute(1, 2, 0).view(bs, c, h, w)
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, entity, triplet, memory,
        ...
        for layer in self.layers:
            output_entity, output_triplet = layer(output_entity, output_triplet, entity_pos, triplet_pos, so_pos,
...
class TransformerDecoderLayer(nn.Module):
    ...
    def entity_decoder(self, tgt_entity, entity_pos, memory, pos, memory_mask, memory_key_padding_mask):
        # 解耦目标的特征向量
    ...
    def forward(self, tgt_entity, tgt_triplet, entity_pos, triplet_pos, so_pos,
        ...
        # 解耦目标的特征向量
        tgt_entity = self.entity_decoder(tgt_entity, entity_pos, memory, pos, memory_mask, memory_key_padding_mask)
        ...
        tgt_sub = self.entity_decoder(tgt_sub, triplet_pos, memory, pos, memory_mask, memory_key_padding_mask)

        # object branch - decoupled visual attention
        tgt_obj = self.entity_decoder(tgt_obj, triplet_pos, memory, pos, memory_mask, memory_key_padding_mask)

        tgt_triplet = torch.cat((tgt_sub, tgt_obj), dim=-1)
        return tgt_entity, tgt_triplet
